...
# Intent taxonomy definition / matching descriptions
INTENT_TAXONOMY = {
    "delivery_delay": "Inquiries regarding package tracking, shipment delays, or missing orders.",
    "payment_issue": "Issues with double charges, billing errors, payment failures, or refund requests.",
    "product_defect": "App crashes, software freezing, broken hardware, or technical glitches.",
...
# Keyword rules for baseline matching (using stems to capture typos, plural, and tenses)
KEYWORD_RULES = {
    "delivery_delay": [
        "deliver", "transit", "where is my order", "package", "shipment", 
        "tracking", "arrive", "not received", "late", "stuck in transit", "delay"
    ],
    "payment_issue": [
        "pay", "charge", "charged", "billing", "refund", "subscription", 
        "deduct", "$", "money", "stuck", "fee", "twice", "overcharge"
    ],
    "product_defect": [
        "freeze", "freezing", "crash", "broken", "update", "issue", 
        "error", "not working", "fault", "bug", "glitch"
    ],
    "general_inquiry": [
        "how to", "what is", "hours", "info", "question", "help", "support"
    ]
}
...
def classify_intent_by_keywords(text: str) -> str:
    ...
    # FIXED: Placed OUTSIDE the loop so all categories are evaluated
    return "other"
...
if __name__ == "__main__":
    test_cases = [
        ...
        "What are your store hours?",
        "My package has not arrived yet",
        "My payment got stucked i din't recieved it yet",
        "Hello!"
    ]
    
    print("Testing Keyword Intent Classifier...")
    for text in test_cases:
        intent = classify_intent_by_keywords(text)
        escalate = intent in ESCALATION_INTENTS
        print(f"\nText: {text}")
